...
final class CGRectTests: XCTestCase {
    ...
    func testCenteredIn() {
        ...
        XCTAssertEqual(rect2.centered(in: rect1), CGRect(origin: origin2, size: rect2.size))
        XCTAssertEqual(rect1.centered(in: rect2), CGRect(origin: origin3, size: rect1.size))

        XCTAssertEqual(rect1.centered(in: rect1), rect1)
    }
